...
void Kinematics::set_motor_param(uint8_t id, float reducation_ratio, uint16_t pulse_ration, float wheel_diameter)
{
    ...
    Serial.printf(
        "init motor param %d: %.6f=%.2f*PI/(%.2f*%u) speed_factor=%lu\n",
        id,
        motor_param_[id].per_pulse_distance,
        wheel_diameter,
        reducation_ratio,
        static_cast<unsigned int>(pulse_ration),
        static_cast<unsigned long>(motor_param_[id].speed_factor));
}
